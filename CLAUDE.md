...
- **Roster is 30 cameras.** Camera ids are POSITIONAL AND UNSTABLE: on 21 Aug "17 Rajkot CCTV" was removed and every id above it shifted down by one, repointing 13 ids at different physical cameras. Key the registry on the **label**; `portal_id` is an attribute, never a key, never a join column. Reconcile every sync by label — see docs/registry/2026-08-21-reconciliation.md.
- Gandhidham Rambaugh p2 is **id 30** (was 31). "17 Rajkot CCTV" **departed**: `portal_id` released to NULL, `last_portal_id = 18`, addressed internally as `offline:17-rajkot-cctv`. It is marked offline and retained, never deleted.
- **`/api/ingest` `location` is a display label, NOT coordinates** ("01 Chiman bhai Bridge"). Zero of the 30 entries carry anything coordinate-shaped. The 11 unlocated cameras therefore gain nothing from the portal and reach `verified` only through the drag-to-place tool.
- Identity has a second, portal-independent signal: a **scene fingerprint** (pHash of a 10 s median frame, bucketed by 30-min slot offset). Measured separation on fixtures: same camera/same bucket 0–14 bits, different cameras 22–38. Label match + hash match reattaches automatically; any mismatch raises an identity conflict that a human confirms, and nothing is reattached until they do. Every reattachment is an audit event with `actor=sync` and its evidence.
